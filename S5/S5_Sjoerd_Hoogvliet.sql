...
WHERE cursus IN (
    SELECT code
    FROM cursussen
    WHERE omschrijving LIKE '%Java%' OR omschrijving LIKE '%XML%'
)
GROUP BY cursist
...
FROM uitvoeringen
WHERE (begindatum BETWEEN '2020-01-01' AND '2020-12-31') AND cursus IN (
    SELECT code
    FROM cursussen
    WHERE type = 'BLD'
)
...
SELECT cursus, begindatum, (
    SELECT COUNT(*)
    FROM inschrijvingen
    WHERE inschrijvingen.cursus = uitvoeringen.cursus AND inschrijvingen.begindatum = uitvoeringen.begindatum
