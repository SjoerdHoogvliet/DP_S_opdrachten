...
SELECT naam
FROM medewerkers
-- Er is bij een tussenvoegsel altijd een spatie aanwezig
WHERE naam LIKE '% %'
...
SELECT u.cursus, u.begindatum, count(i.*) AS aantal_inschrijvingen
FROM uitvoeringen u
...
SELECT (MAX(gbdatum) - MIN(gbdatum)) / 365 AS verschil, FLOOR(AVG((NOW()::date - gbdatum) / 365)) AS gemiddeld
FROM medewerkers
...
-- Aangezien de AVG functie null waarden niet meeneemt heb ik een eigen berekening gemaakt
SELECT COUNT(*) AS aantal_medewerkers, SUM(comm) / COUNT(*) AS commissie_medewerkers, AVG(comm) FILTER (WHERE functie = 'VERKOPER') AS commissie_verkopers
FROM medewerkers
